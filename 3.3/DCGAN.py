...
class CrypkoDataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        img = torchvision.io.read_image(fname)# (C,H,W)
...
def get_dataset(root):
    fnames = glob.glob(os.path.join(root, '*'))#返回路径列表
    compose = [
        transforms.ToPILImage(),
        transforms.Resize((64, 64)),
        transforms.ToTensor(),#(C,H,W)+归一化
...
# plt.imshow(grid_image.permute(1,2,0))# 需要（H,W,C)
# plt.show()

class Generator(nn.Module):
    #input (bathch,in_dim) output(batch,3,64,64)
    def __init__(self, in_dim, feature_dim = 64):
        ...
        self.l3 = nn.Sequential(
            nn.ConvTranspose2d(feature_dim, 3, kernel_size=5, stride=2,
                               padding=2, output_padding=1, bias=False),        #(batch, 3, 64, 64)
            nn.Tanh()   
        )
        self.apply(weights_init)    

    def dconv_bn_relu(self, in_dim, out_dim):#转置卷积增加特征图的空间尺寸
...
class Discriminator(nn.Module):
    ...
    def __init__(self, in_dim, feature_dim=64):
        ...
        self.l1 = nn.Sequential(
            ...
            self.conv_bn_lrelu(feature_dim * 4, feature_dim * 8),               #(batch, 512, 4, 4)
...
class TrainerGAN():
    def __init__(self, config):
        ...
        self.steps = 0
        self.z_samples = Variable(torch.randn(100, self.config["z_dim"])).cuda()#每一维度服从正态分布
    ...
    def train(self):
        ...
        for e, epoch in enumerate(range(self.config["n_epoch"])):
            ...
            for i, data in enumerate(progress_bar):
                ...
                r_loss = self.loss(r_logit, r_label)#BCE LOSS = -(ylog(p)+(1-y)log(1-p))y是标签，对所有样本取平均
                f_loss = self.loss(f_logit, f_label)
                loss_D = (r_loss + f_loss) / 2#最小化此损失 r1,f0
                ...
                # *    Train G        *
                # *********************
                if self.steps % self.config["n_critic"] == 0:#generator更新频率比discriminator慢
...
if __name__ == '__main__':
    ...
    trainer.train()
    # G_path = 'D:\Lihongyi\GAN\checkpoints\2025-02-26_21-34-34_GAN\G_99.pth'
    # trainer.inference('D:\\Lihongyi\\GAN\\checkpoints\\2025-02-26_21-34-34_GAN\\G_99.pth', n_generate=1000, n_output=30, show=True)